...
class BaseSolver:
    def __init__(self):
        ...
        self.group2list = {}
        return  
    def add_group(self, group, stock):
        if group not in self.group2list:
            self.group2list[group] = []
        self.group2list[group].append(stock)
        return 
...
class industrySolver(BaseSolver):
    def __init__(self): 
        ...
        return 
    def filter(self, stock):
        return stock.ts_code in self.stock2open_close_before
        return True
        try:
            open, close = self.stock2open_close_before[stock.ts_code] 
            return  close / open -1 > 0.05
        except:
            return False
    ...
    def grouper(self, stocks):
        ...
        """
        group_key = 'industry'  
        for i, cur_stock in enumerate(stocks):
            group = getattr(cur_stock, group_key) 
            self.add_group(group, cur_stock)

        return {g : self.group2list[g] for g in self.group2list if len(self.group2list[g]) > 50}

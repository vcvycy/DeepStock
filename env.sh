...
alias stock_update_train_avg_label="cd $STOCK_HOME; python -m train.update_avg_label_table"
...
cat <<EOF
 可使用的命令:
    ...
    (3) stock_train: 训练模型
    (4) stock_update_train_avg_label 依赖于训练数据, 更新date2avg_label/fid2avg_label
EOF